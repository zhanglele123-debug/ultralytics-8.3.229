...
def create_data_yaml(data_dir="datasets/icon"):
    """功能：自动扫描标签文件，生成YOLO训练必需的配置文件 适配：640×320分辨率图标数据集.
    """
    # 训练/验证标签路径
...
def train_yolo11n():
    """核心优化： 1. 兼容 ultralytics 8.3.229 版本（删除无效参数） 2. 适配 640×320 分辨率图片（等比例训练，不变形） 3. 彻底解决Windows训练曲线跳变、震荡问题 4. 小样本图标专用优化.
    """
    # 数据集路径
